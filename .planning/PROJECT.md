...
- **Success metric**: 64 条新内容和 2,100 条存量修复全部通过线上数量、页面字段、分类与 URL 稳定性验收
- **Strategy notes**: `/Users/longnv/bin/repo/fastgpt-data/W2-内容方向与首批内容-20260730/`

## Current Milestone: v1.0 W2 官网内容上线

**Goal:** 将 60 条精选规范 FAQ、4 篇竞品对比页、100 条存量 FAQ Meta 与 2,000 条分类重挂完整发布到官网，并建立可重复执行的验证与回滚流程。
...
- 将首批 100 条 Title/Description 绑定到对应存量 FAQ，并保持正文与 URL 原样
- 将 2,000 条分类建议重挂到 9 类，并保留置信度、人工复核与批次证据
...
- [ ] 确定性导入 60 条 W2 V1.1 中文 FAQ，并验证 60 条、14 类、字段完整、slug 唯一与源内容一致
- [ ] 将静态参数和 sitemap 从英文 key 集合扩展为 locale-aware 路由集合
- [ ] 校准 `fastgpt.cn` 与 `fastgpt.io/zh` 的 canonical、hreflang、robots 与 sitemap 规则
- [ ] 建立竞品对比内容模型、页面模板、4 篇正文发布流程与 90 天复核字段
- [ ] 建立存量 FAQ 身份对齐层，将 100 条 Meta 和 2,000 条分类建议映射到真实线上对象
- [ ] 建立批量导入、审计、回滚与线上验收流程，覆盖精确数量和 URL 稳定性
...
- 竞品页源文件保留产品、销售与法务签发要求；生产发布需要记录对应签发证据。
...
| 存量 FAQ 导入先完成身份对齐审计 | 仓内 1,400 条与交付包 2,000 行存在真实数据边界差异 | — Pending validation |
| 竞品页发布保留三方签发证据 | 源稿明确记录产品、销售与法务签发要求 | — Pending validation |
...
---
*Last updated: 2026-08-04 after starting milestone v1.0*
