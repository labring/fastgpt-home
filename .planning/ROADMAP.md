...
本里程碑先建立可审计的源数据与内容身份基线，再依次交付新增 FAQ、竞品对比页和存量 FAQ 修复，最后以自动化、人工与线上证据完成直发验收。五个阶段共同守住内容一致性、存量 URL 稳定性、双域 SEO 信号和批量操作可回滚性，延期源行保留在交接包中。

## Milestone v1.0 W2 官网内容上线

**Milestone Goal:** 将 60 条精选规范 FAQ、4 篇竞品对比页以及可唯一映射的存量 Meta 与分类修复直发到官网，并建立可重复执行的验证与回滚流程。
...
- [x] **Phase 3: 竞品对比页发布** - 在保留签发字段与复核信息的基础上直发四篇信息完整、响应式且可持续复核的竞品对比页。
- [x] **Phase 4: 存量 FAQ 批量修复** - 完成 76 条 Meta 与 1,394 条分类的运行时覆盖，保全正文和 URL，并记录 24/606 条延期源行。
- [x] **Phase 5: 整批发布验收与交接** - 以构建、自动化、浏览器、生产工作流和线上证据验收 64 个目标 URL；直发豁免与延期项进入交接包。
...
**Success Criteria** (what must be TRUE):
  ...
  5. 产品、销售与法务签发字段、复核日期和来源信息保留在页面与清单中；用户授权的直发豁免可发布对应页面并保留审计记录。
**Plans**: 5 plans
...
**Success Criteria** (what must be TRUE):
  1. 76 条可唯一映射的 Meta 精确绑定对应线上 FAQ，页面在 HTML、Open Graph 与 Twitter 中呈现各自的 Title 和 Description；24 条源行保留延期记录。
  2. 可唯一映射的分类建议产生九类体系下的导入结果；606 条缺少运行时对象的源行保留置信度、人工复核状态、原分类和新分类记录。
...
**Success Criteria** (what must be TRUE):
  ...
  4. 线上证据证明 64 个已发布目标 URL 可访问、内容版本正确、搜索引擎可抓取且中文 canonical 指向 `fastgpt.cn`。
...
| 3. 竞品对比页发布 | 5/5 | Complete (direct-published with waived signoff findings) | 2026-08-05 |
| 4. 存量 FAQ 批量修复 | 3/3 | Complete (76 Meta and 1,394 category mappings live; 24/606 deferred) | 2026-08-05 |
| 5. 整批发布验收与交接 | 2/2 | Complete (workflow and 64 live URL checks passed) | 2026-08-05 |
