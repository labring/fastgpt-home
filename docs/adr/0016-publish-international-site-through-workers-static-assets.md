...
ADR 0007 assigns the China Site to Docker/Nginx and keeps publication at the Cloudflare edge. The International Site already exports a generated Worker for the URL Alias Authority projection. Workers Static Assets can publish that Worker and the complete static export as one release unit while preserving the China Site's Docker/Nginx path. Per-PR Worker previews are recorded separately in ADR 0017.
...
- Keep the China Site on Docker/Nginx. Custom Domains and DNS cutover for `fastgpt.io` require a later release decision. Preview Worker publication follows ADR 0017.
...
The first deployment is reachable through `workers.dev` while production DNS continues to serve the retained Pages publication. A failed comparison can return to Pages without changing the China Site path or the PR Preview Worker path.
