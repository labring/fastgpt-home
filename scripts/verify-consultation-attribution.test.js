...
test('reused contact anchors retain configured source but refresh inherited source', () => {
  const env = browser();
  const script = env.load('src/lib/contactLinkAttribution.ts').contactLinkAttributionScript;
  vm.runInContext(script, env.context);

  const configured = new env.Element('/contact?source=customers');
  env.window.location = new URL('https://fastgpt.cn/customers?source=partner-a');
  env.listeners.click({ target: configured });
  env.window.location = new URL('https://fastgpt.cn/customers?source=partner-b');
  env.listeners.click({ target: configured });
  assert.equal(
    new URL(configured.href, env.window.location.origin).searchParams.get('source'),
    'customers'
  );

  const inherited = new env.Element('/contact#form');
  env.window.location = new URL('https://fastgpt.cn/customers?source=partner-a');
  env.listeners.click({ target: inherited });
  assert.equal(
    new URL(inherited.href, env.window.location.origin).searchParams.get('source'),
    'partner-a'
  );
  env.window.location = new URL('https://fastgpt.cn/customers?source=partner-b');
  env.listeners.click({ target: inherited });
  ...
  inherited.setAttribute('href', '/contact?source=customers#form');
  env.window.location = new URL('https://fastgpt.cn/customers?source=partner-c');
  env.listeners.click({ target: inherited });
  assert.equal(
    new URL(inherited.href, env.window.location.origin).searchParams.get('source'),
    'customers'
