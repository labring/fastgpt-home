...
function checkHtml(body, expected, host, hosts) {
  ...
  const alternateTags = [...body.matchAll(/<link\b[^>]*rel=["']alternate["'][^>]*>/gi)].map((match) => [attr(match[0], 'hreflang'), attr(match[0], 'href')]);
  const alternate = Object.fromEntries(alternateTags); const requiredLanguages = ['zh-CN', 'en', 'x-default'];
  if (alternateTags.length !== requiredLanguages.length || new Set(alternateTags.map(([language]) => language)).size !== requiredLanguages.length || requiredLanguages.some((language) => !alternate[language])) errors.push('alternates:keys');
  ...
  const robots = [...body.matchAll(/<meta\b[^>]*>/gi)].find((match) => attr(match[0], 'name')?.toLowerCase() === 'robots');
  ...
  for (const [variant, expected] of Object.entries(matrix)) {
    ...
    for (const expectedRoute of expected.routes) {
      ...
      try { const response = await fetchWithTimeout(url, options.timeoutMs, fetchImpl); const body = await response.text(); item.status = response.status; item.finalUrl = response.url; item.headers = headersOf(response.headers); item.bodyDigest = crypto.createHash('sha256').update(body).digest('hex'); const surfaces = [];
        if (response.status !== 200) surfaces.push(`status=${response.status}`); if (response.url !== url) surfaces.push('redirect'); if (response.status === 200) surfaces.push(...checkHtml(body, expectedRoute, host, { cn: options.baseUrlCn, io: options.baseUrlIo })); if (!sitemap.includes(`<loc>${host}${expectedRoute.route}</loc>`)) surfaces.push('sitemap'); if (!/max-age=\d*[1-9]/i.test(item.headers['cache-control'] || '') && !item.headers['cf-cache-status'] && !item.headers['x-cache-status']) surfaces.push('cache'); if (surfaces.length) { item.failures = surfaces; result.failures.push(`${expectedRoute.route}:${surfaces.join(',')}`); } } catch (error) { item.failures = [error.name]; result.failures.push(`${expectedRoute.route}:${error.name}`); }
