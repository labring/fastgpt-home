...
const faqLinkLabel: Record<string, string> = {
  zh: 'FAQ',
  en: 'FAQ',
  ja: 'FAQ'
};
...
export default async function LangHome({ children, params }: { children: React.ReactNode, params: Promise<{ lang?: string }> }) {
  ...
  const headerDict = { ...dict, links: [...dict.links] };
  if (showFAQ) {
    headerDict.links.push({
      label: faqLinkLabel[langName] || 'FAQ',
      href: '/faq'
    });
  }
  ...
  return (
    <>
      ...
      <Header dict={headerDict} CTALocale={dict.CTAButton} />
