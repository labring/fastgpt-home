...
function extractSlug(rawText: string) {
  ...
  return normalizeSolutionSlug(firstLine.replace(/`/g, '').replace(/^slug\s*:\s*/i, ''));
}
...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You generate concise English URL slugs for solutions or customer case studies. Return exactly one slug. Use lowercase English words, numbers only if necessary, and hyphens between words. Do not return Chinese, pinyin, quotes, markdown, JSON, or explanations.'
        },
        {
          role: 'user',
          content: `Solution or case title: ${title}`
        }
      ],
      stream: false,
      temperature: 0.2
    ...
    if (!response.ok) {
      ...
      console.error('AI solution slug suggestion failed:', errorText);
    ...
    if (!slug || !isValidSolutionSlug(slug)) {
    ...
    console.error('Error suggesting solution slug:', error);
