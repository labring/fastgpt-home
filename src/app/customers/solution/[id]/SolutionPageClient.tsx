...
import HelpfulFeedback from "@/customers/components/solution/HelpfulFeedback";
import { withBasePath } from "@/customers/lib/base-path";

const AiSummaryCard = lazy(() => import("@/customers/components/solution/AiSummaryCard"));
...
import { publishSolutionInteractionPatch } from "@/customers/lib/solution-interaction-events";
...
const allSolutionsRequestUrl = withBasePath('/api/customers?limit=100');

function patchSolutionInList(
  currentData: { solutions?: CardSolution[] } | undefined,
  solutionId: string,
  patch: Partial<CardSolution>
) {
  if (!currentData?.solutions) {
    return currentData;
  }

  return {
    ...currentData,
    solutions: currentData.solutions.map((item) => (
...
export default function SolutionPageClient({ id, initialSolution, initialAllSolutions = [] }: SolutionPageClientProps) {
  const [localLikes, setLocalLikes] = useState(initialSolution?.likes ?? 0);
  const [isLiked, setIsLiked] = useState(initialSolution?.isLiked ?? false);
  const [localUsage, setLocalUsage] = useState<string>(initialSolution?.usage ?? "");
  const [hasViewed, setHasViewed] = useState(Boolean(initialSolution?.hasViewed));
  ...
  const likedStickyRef = useRef(Boolean(initialSolution?.isLiked));
  const likesFloorRef = useRef<number | null>(null);
  const usageFloorRef = useRef<number | null>(null);
  const solutionIdRef = useRef(id);
  const router = useRouter();
  const { mutate } = useSWRConfig();

  const { data: solution, error } = useSWR<SolutionDetail>(
    id ? withBasePath(`/api/customers/${id}`) : null,
    noStoreJsonFetcher,
    {
      fallbackData: initialSolution || undefined,
      revalidateOnFocus: true,
      revalidateOnReconnect: true,
      dedupingInterval: 10000,
      onSuccess: (data) => {
        if (solutionIdRef.current !== id) {
        ...
        if (id) {
          const localState = getLikedSolutionState(id);
          const viewedState = getViewedSolutionState(id);
  ...
  const { data: allSolutionsData } = useSWR(allSolutionsRequestUrl, noStoreJsonFetcher, {
    fallbackData: initialAllSolutions.length > 0 ? { solutions: initialAllSolutions } : undefined,
    revalidateOnFocus: true,
    revalidateOnReconnect: true,
    dedupingInterval: 10000,
  });
  ...
  const prevSolution = useMemo(() => {
    if (!solution || navigationSolutions.length === 0) return null;
    const currentIndex = navigationSolutions.findIndex((item) => String(item.id) === String(solution.id));
    const safeCurrentIndex = currentIndex >= 0 ? currentIndex : 0;
    const previousIndex = (safeCurrentIndex - 1 + navigationSolutions.length) % navigationSolutions.length;
  ...
  const allRelatedSolutions = useMemo(() => {
    if (!solution || !allSolutionsData?.solutions) return [];
  ...
  const handleCategoryClick = useCallback((categoryId: string) => {
    const categorySlug =
      solution?.categoryId === categoryId ? solution.categorySlug : undefined;
  ...
  const handleRelatedLikeToggle = useCallback((solutionId: string | number, state?: { isLiked: boolean; likes: number }) => {
    if (!state) {
      return;
    }

    void mutate(allSolutionsRequestUrl, (currentData: { solutions?: CardSolution[] } | undefined) => (
      patchSolutionInList(currentData, String(solutionId), state)
    ), { revalidate: false });
  }, [mutate]);
  ...
  const handleSmartSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (data.matched_case) {
         ...
         let matchedSolution: CardSolution | null = null;
         try {
           matchedSolution = await fetchFirstMatchedSolution<CardSolution>(data.matched_case);
         } catch (e) {
           console.error('Failed to fetch matched solution detail', e);
         }
  ...
  const handleLikeToggle = useCallback(async (e: React.MouseEvent) => {
    ...
    setIsLiked(true);
    likesFloorRef.current = Math.max(localLikes, likesFloorRef.current ?? solution?.likes ?? initialSolution?.likes ?? 0);
    saveLikedSolutionState(id, true);
    ...
    try {
      ...
      if (res.ok && data?.success && typeof data.likes === 'number') {
        ...
        saveLikedSolutionState(id, true);
        void mutate(detailRequestUrl, (currentData: SolutionDetail | undefined) => (
          currentData
            ? { ...currentData, likes: confirmedLikes, isLiked: true }
            : currentData
        ), { revalidate: false });
        void mutate(allSolutionsRequestUrl, (currentData: { solutions?: CardSolution[] } | undefined) => (
          patchSolutionInList(currentData, id, { likes: confirmedLikes, isLiked: true })
        ), { revalidate: false });
      ...
      const fallbackLiked = solution?.isLiked ?? initialSolution?.isLiked ?? false;
      const fallbackLikes = solution?.likes ?? initialSolution?.likes ?? 0;
      likedStickyRef.current = fallbackLiked;
      likesFloorRef.current = null;
      setIsLiked(fallbackLiked);
      setLocalLikes(fallbackLikes);
      saveLikedSolutionState(id, fallbackLiked);
    } finally {
      setIsLikePending(false);
    }
  }, [id, initialSolution?.isLiked, initialSolution?.likes, isLiked, isLikePending, localLikes, mutate, solution?.isLiked, solution?.likes]);
  ...
  const countView = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data?.success && typeof data.rawUsageCount === 'number') {
        ...
        saveViewedSolutionState(id, true);
        void mutate(detailRequestUrl, (currentData: SolutionDetail | undefined) => (
          currentData
            ? {
                ...currentData,
        ...
        void mutate(allSolutionsRequestUrl, (currentData: { solutions?: CardSolution[] } | undefined) => (
          patchSolutionInList(currentData, id, {
            usage: nextUsage,
            rawUsageCount: usageFloorRef.current ?? data.rawUsageCount,
            hasViewed: true
          })
          ...
          <div className="w-full bg-white/0 dark:bg-black/0 backdrop-blur-2xl border-b border-transparent pointer-events-auto transform-gpu">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:pl-4 lg:pr-8 pt-3 pb-3">
              <nav className="flex items-center text-sm font-medium text-ink-sub dark:text-[#aeb4bc] w-fit">
                ...
                <span className="mx-2">/</span>
                <Link href={`/customers/categories/${solution.categorySlug}`} className="hover:text-brand-600 transition-colors">
            ...
            <div className="flex flex-col lg:flex-row items-start relative w-full">
              ...
              <article ref={articleRef} className="min-w-0 flex-1 w-full transition-all duration-500 ease-in-out">
                ...
                <Suspense fallback={null}>
                  <AiSummaryCard
                    key={`${id}:${solution.updatedAt || 'current'}`}
                    solutionId={id}
                    contentVersion={solution.updatedAt}
                  />
                </Suspense>
