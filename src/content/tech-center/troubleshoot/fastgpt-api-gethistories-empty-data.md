---
title: 解决FastGPT部分应用调用getHistories接口无历史记录的问题
slug: /zh/troubleshoot/fastgpt-api-gethistories-empty-data
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5939
source_type: GitHub issue
---

# 解决FastGPT部分应用调用getHistories接口无历史记录的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.8.18；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT私有部署v4.8.18版本中，调用官方提供的`/api/core/chat/getHistories`接口时，部分指定`appid`无法返回对话历史记录；更换其他`appid`后，接口可正常返回对应数据，且前端可查看到对应API调用的对话日志。同时使用`/api/core/chat/getPaginationRecords`接口时，也存在同类异常情况。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5939)
