---
title: 解决FastGPT Api知识库问答拆分导入时报错的问题
slug: /zh/troubleshoot/fastgpt-api-kb-qa-import-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4093
source_type: GitHub issue
---

# 解决FastGPT Api知识库问答拆分导入时报错的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.23；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 使用FastGPT公有云4.8.23版本的Api文件库类型知识库时，采用问答拆分形式导入数据会触发报错。系统日志会反复出现`message: 'e is not async iterable'`的错误堆栈，且导入数据的状态一直停留在索引中。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4093)
