---
title: 解决FastGPT v4.9.14版本对话API传入messages无法构建上下文问题
slug: /zh/troubleshoot/fastgpt-api-messages-context-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5148
source_type: GitHub issue
---

# 解决FastGPT v4.9.14版本对话API传入messages无法构建上下文问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.9.14；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT v4.9.14版本中调用对话API时，传入了符合OpenAI官方chat API格式的messages参数，参数中包含完整的历史对话上下文信息，但FastGPT未按照该参数构建对话上下文，模型输出未关联传入的历史内容。查看FastGPT运行日志，确认请求已被接收且格式符合要求，但上下文未生效。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5148)
