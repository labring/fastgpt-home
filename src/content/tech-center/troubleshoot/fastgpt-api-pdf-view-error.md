---
title: 解决FastGPT接口上传PDF后查看原始内容失败的问题
slug: /zh/troubleshoot/fastgpt-api-pdf-view-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3274
source_type: GitHub issue
---

# 解决FastGPT接口上传PDF后查看原始内容失败的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.8.10；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT v4.8.10私有部署版本中，通过`/api/core/dataset/collection/create/localFile`接口上传知识库PDF文件后，点击“查看原始内容”按钮，报错“未能加载PDF文档”。使用手动上传的同类型PDF文件，则可以正常查看原始内容。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3274)
