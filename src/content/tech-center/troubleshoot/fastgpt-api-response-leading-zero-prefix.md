---
title: 解决FastGPT调用非函数调用模型时API响应以0: 开头的问题
slug: /zh/troubleshoot/fastgpt-api-response-leading-zero-prefix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2340
source_type: GitHub issue
---

# 解决FastGPT调用非函数调用模型时API响应以0: 开头的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.8；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.8.8私有部署版本中，创建配置了工具调用的智能体并选择不支持函数调用的qwen2模型，发布智能体API后，使用Python的AsyncOpenAI库发起非流式请求，响应内容会以"0: "作为前缀。例如使用代码`import asyncio; from openai import AsyncOpenAI; client = AsyncOpenAI(); async def main(): chat_completion = await client.cha
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2340)
