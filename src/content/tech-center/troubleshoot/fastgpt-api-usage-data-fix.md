---
title: 解决FastGPT调用应用API时usage数据固定为1的问题
slug: /zh/troubleshoot/fastgpt-api-usage-data-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3861
source_type: GitHub issue
---

# 解决FastGPT调用应用API时usage数据固定为1的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.22、2.5-1b；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT V4.8.22私有部署版本中，通过API方式调用引用了自定义模型（如qwen2.5-1b）的应用时，返回的API结果中usage数据全部固定为1。前端页面的日志可显示正确的token计数，但直接调用FastGPT应用API时无法获取模型实际返回的usage数据。同时直接请求模型提供商的原生API，可正确返回符合实际调用量的usage数据。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3861)
