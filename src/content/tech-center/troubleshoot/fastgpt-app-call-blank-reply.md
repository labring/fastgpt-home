---
title: 解决FastGPT私有部署版本应用间调用返回空白的问题
slug: /zh/troubleshoot/fastgpt-app-call-blank-reply
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1851
source_type: GitHub issue
---

# 解决FastGPT私有部署版本应用间调用返回空白的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v5.8.5；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT v5.8.5私有部署版本中，用户在一个应用内完成分类后调用另一个应用时，出现回复空白的情况。单独调用被调用的应用可正常返回结果，且调用过程的日志未显示明确报错信息。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1851)
