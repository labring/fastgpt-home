---
title: 解决FastGPT跨环境应用导入时MongoDB主键_id丢失的问题
slug: /zh/troubleshoot/fastgpt-app-import-id-retention
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4981
source_type: GitHub issue
---

# 解决FastGPT跨环境应用导入时MongoDB主键_id丢失的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.9.10；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT v4.9.10版本的公有云或私有部署环境中，从开发环境工作台导出的简易应用、工作流、插件等配置JSON，导入到测试或生产环境后，会丢失开发环境MongoDB的apps表中原有的主键`_id`值，系统自动生成新的`_id`。若开发环境的工作流引用了特定插件，导入后插件的`_id`发生变更，会导致工作流打开时弹出"工具不存在"的提示，需手动修改引用关系或重新关联插件，操作繁琐且随着工作流节点量增加愈发不便。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4981)
