---
title: 解决FastGPT自动分块时三级标题字符不足丢失的问题
slug: /zh/troubleshoot/fastgpt-auto-chunking-title-missing
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1429
source_type: GitHub issue
---

# 解决FastGPT自动分块时三级标题字符不足丢失的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.7.1-alpha2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT私有部署版v4.7.1-alpha2中使用自动分块功能时，当三级标题（以### 开头）下方的字符数少于29个，该三级标题会丢失，其下方的内容会直接落入下一个分块。根据用户提供的测试结果，当三级标题下方字符数为28个时仍会触发该错误，达到29个时分块结果恢复正常。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1429)
