---
title: 解决FastGPT使用Azure OpenAI模型时文本抽取的参数报错问题
slug: /zh/troubleshoot/fastgpt-azure-openai-function-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/574
source_type: GitHub issue
---

# 解决FastGPT使用Azure OpenAI模型时文本抽取的参数报错问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 3.5-turbo；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用私有部署版本的FastGPT，搭配Azure OpenAI的gpt-3.5-turbo-16k模型，在高级编排中添加文本抽取模块时，触发报错`Unrecognized request argument supplied: functions`，预期可正常完成文本内容提取操作。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/574)
