---
title: 解决FastGPT批量运行HTTP请求失败及错误传递问题
slug: /zh/troubleshoot/fastgpt-batch-http-error-transfer
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5460
source_type: GitHub issue
---

# 解决FastGPT批量运行HTTP请求失败及错误传递问题
...
> 用户在FastGPT批量运行功能中使用HTTP请求节点时，出现两个问题：一是某次循环内HTTP请求失败后，该循环直接终止，HTTP请求的后续节点不再执行；二是请求失败的错误信息会被带入下一次循环，导致后续循环受上一次的错误状态影响。例如构造包含`ping`、`test`、`mtr`的数组时，第二个元素`test`为HTTP请求不支持的类型，第二次循环HTTP请求失败后直接结束，错误信息被带入第三次循环，使第三次循环错误进入失败分支。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5460)
