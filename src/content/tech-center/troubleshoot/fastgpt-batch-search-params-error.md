---
title: 解决FastGPT中批量搜索调用参数异常导致结果错误的问题
slug: /zh/troubleshoot/fastgpt-batch-search-params-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3253
source_type: GitHub issue
---

# 解决FastGPT中批量搜索调用参数异常导致结果错误的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.13；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT 4.8.13私有部署版本中，发送包含阿苯达唑胶囊、阿苯达唑颗粒两个药品通用名的批量搜索请求后，系统调用了两次搜索接口，但返回结果不正确。对话框中显示的请求参数正确匹配输入的查询内容，但查看请求详情时，两次搜索的实际参数完全一致，无法区分两个不同的通用名查询需求。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3253)
