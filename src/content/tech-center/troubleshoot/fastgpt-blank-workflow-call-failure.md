---
title: 解决FastGPT空白工作流在聊天界面无法正常调用的问题
slug: /zh/troubleshoot/fastgpt-blank-workflow-call-failure
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5265
source_type: GitHub issue
---

# 解决FastGPT空白工作流在聊天界面无法正常调用的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.10.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT中创建空白工作流并完成配置后，返回聊天界面调用该工作流时无任何返回内容，且工作流无执行记录。使用官方默认模板（如问题分类+知识库）创建的工作流可正常调用，该问题在公有云版本与v4.10.0私有部署版本均存在。用户提供的截图显示，空白工作流调用时无任何节点执行的日志记录，而模板工作流调用后可正常展示完整的执行流程与结果。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5265)
