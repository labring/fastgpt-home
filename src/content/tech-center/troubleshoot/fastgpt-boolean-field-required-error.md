---
title: 解决FastGPT布尔类型字段设为必填后的界面与输出异常问题
slug: /zh/troubleshoot/fastgpt-boolean-field-required-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4462
source_type: GitHub issue
---

# 解决FastGPT布尔类型字段设为必填后的界面与输出异常问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.9.2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT私有部署v4.9.2版本中，配置工作流提取布尔类型字段`is_success`时，出现两类异常： 1. 当该字段设为非必填时，流程输出符合预期；将其设置为必填字段后，流程输出结果异常。 2. 布尔类型字段设置为必填后，默认值的输入框未显示为选择框，不符合预期的组件样式。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4462)
