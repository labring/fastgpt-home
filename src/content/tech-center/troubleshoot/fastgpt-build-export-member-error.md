---
title: 解决FastGPT 4.8.11版本打包缺失导出成员报错问题
slug: /zh/troubleshoot/fastgpt-build-export-member-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2902
source_type: GitHub issue
---

# 解决FastGPT 4.8.11版本打包缺失导出成员报错问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.11-fix；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在私有部署FastGPT 4.8.11-fix版本时，执行docker build命令出现打包报错。具体错误信息为： ``` Type error: Module '"@fastgpt/global/core/workflow/template/system/aiChat"' has no exported member 'AiChatQuotePrompt'. ``` 报错位置为src/pages/app/detail/components/WorkflowCompon
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2902)
