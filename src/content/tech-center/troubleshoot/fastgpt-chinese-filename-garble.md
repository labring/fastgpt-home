---
title: 解决FastGPT 4.8.11私有部署的中文文件名乱码问题
slug: /zh/troubleshoot/fastgpt-chinese-filename-garble
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3601
source_type: GitHub issue
---

# 解决FastGPT 4.8.11私有部署的中文文件名乱码问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.11；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT 4.8.11私有部署环境中，通过Golang接口调用上传本地文件集合时，中文文件名出现乱码，但上传的文件内容保持正确。请求体表单中明确携带`filename="测试文档.txt"`的参数，但最终展示的文件名存在乱码。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3601)
