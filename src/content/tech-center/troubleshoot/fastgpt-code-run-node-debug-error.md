---
title: 解决FastGPT代码运行节点流程调试报错inputString.replace问题
slug: /zh/troubleshoot/fastgpt-code-run-node-debug-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3343
source_type: GitHub issue
---

# 解决FastGPT代码运行节点流程调试报错inputString.replace问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.14；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT V4.8.14私有部署版本中，使用流程整体调试（页面右上角运行或从"流程开始"节点点击调试）时，代码运行节点会抛出`inputString.replace is not a function`报错。仅在代码运行节点单独点击调试时可正常运行。导入提供的流程测试文件后，输入文本“啊啊啊，aa,b,存储，d”，流程调试会触发该报错，单独调试节点可正常输出预期的分割数组结果。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3343)
