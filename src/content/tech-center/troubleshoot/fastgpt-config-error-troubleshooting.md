---
title: FastGPT私有部署模型配置错误的排查与解决
slug: /zh/troubleshoot/fastgpt-config-error-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2186
source_type: GitHub issue
---

# FastGPT私有部署模型配置错误的排查与解决

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.8.7；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用v4.8.7版本的FastGPT私有部署版本，在配置config.json文件后出现服务启动或模型调用报错。从用户提供的配置片段可见，llmModels数组的最后一个模型配置条目未完整编写，仅写到"customExtract"字段就中断，未完成字段赋值、对象闭合及数组收尾，导致JSON文件格式不合法。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2186)
