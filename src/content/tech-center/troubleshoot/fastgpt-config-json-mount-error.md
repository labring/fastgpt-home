---
title: 解决FastGPT容器挂载config.json文件启动失败的问题
slug: /zh/troubleshoot/fastgpt-config-json-mount-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3624
source_type: GitHub issue
---

# 解决FastGPT容器挂载config.json文件启动失败的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.17；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT 4.8.17版本在使用Docker部署时，若在docker-compose配置中添加`volumes: - ./config.json:/app/data/config.json`挂载项，会导致容器启动失败。终端会输出如下报错信息： ``` Error response from daemon: failed to create task for container: failed to create shim task: OCI runtime create
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3624)
