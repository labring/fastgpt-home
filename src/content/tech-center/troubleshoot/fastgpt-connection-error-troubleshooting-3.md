---
title: 解决FastGPT私有部署调用大模型出现Connection error的问题
slug: /zh/troubleshoot/fastgpt-connection-error-troubleshooting-3
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5159
source_type: GitHub issue
---

# 解决FastGPT私有部署调用大模型出现Connection error的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.9.4；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT私有部署V4.9.4版本，通过Docker部署运行三个月且未修改配置，突然出现调用大模型API时提示“Connection error.”的情况，即使是最简单的聊天机器人也会触发该报错。用户已确认配置的密钥和API地址可正常使用。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5159)
