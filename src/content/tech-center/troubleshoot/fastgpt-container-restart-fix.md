---
title: 解决FastGPT部署后pg、aiproxy容器启动失败反复重启问题
slug: /zh/troubleshoot/fastgpt-container-restart-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4283
source_type: GitHub issue
---

# 解决FastGPT部署后pg、aiproxy容器启动失败反复重启问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.30.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用最新版docker-compose.yml文件，将镜像源替换为阿里云，在Win10系统搭配Docker Desktop v4.30.0的环境中执行docker compose up -d命令后，pg、aiproxy、aiproxy_pd容器反复重启，日志输出异常。用户同时上传了aiproxy_pg-log、apiproxy-log、fastgpt-log、mongo-log、pg-log、sandbox-log共6份日志文件。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4283)
