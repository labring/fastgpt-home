---
title: 解决FastGPT自定义模型地址无法正常调用的问题
slug: /zh/troubleshoot/fastgpt-custom-address-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3675
source_type: GitHub issue
---

# 解决FastGPT自定义模型地址无法正常调用的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.20-alpha；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 私有部署的FastGPT V4.8.20-alpha版本中，配置自定义模型地址时出现报错。具体表现为无可用通道、模型为空、返回404错误。用户尝试过多个官方地址，包括https://api.deepseek.com/v1、https://api.deepseek.com、https://api.deepseek.com/v1/chat/completions，其他第三方模型服务的自定义地址也无法正常使用。在其他调用场景中该地址可正常调用，但直接在FastGPT中配置自定义地址
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3675)
