---
title: FastGPT自定义前端调用对话API的参数配置与403报错排查
slug: /zh/troubleshoot/fastgpt-custom-api-params-403-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3116
source_type: GitHub issue
---

# FastGPT自定义前端调用对话API的参数配置与403报错排查

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.9；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在私有部署版本V4.8.9的FastGPT中，基于官方对话API文档重构自定义前端页面时，遇到以下问题： 1. 调用getHistories接口时，不清楚appId和source参数的取值来源； 2. 未携带cookie调用getHistories接口，返回403报错，返回内容为： ```json { "code": 403, "statusText": "unAuthorization", "message": "common:code_error.error_mess
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3116)
