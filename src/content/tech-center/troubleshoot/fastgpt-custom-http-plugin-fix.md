---
title: FastGPT调用自定义HTTP插件失败的排查与解决方法
slug: /zh/troubleshoot/fastgpt-custom-http-plugin-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1347
source_type: GitHub issue
---

# FastGPT调用自定义HTTP插件失败的排查与解决方法

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.7.1、4.8；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT 4.7.1或4.8 preview4版本中，完成自定义工具创建与HTTP插件配置后，发起提问调用该插件时出现调用失败的情况，附带多张报错截图。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1347)
