---
title: 解决FastGPT私有部署下自定义密钥连接模型失败的问题
slug: /zh/troubleshoot/fastgpt-custom-key-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1168
source_type: GitHub issue
---

# 解决FastGPT私有部署下自定义密钥连接模型失败的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.6.8；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT私有部署v4.6.8环境中，通过docker启动模型服务容器并配置了`-e SK-KEY=[REDACTED_CREDENTIAL]`参数，进入容器执行`docker exec m3e env`命令，确认SK-KEY=[REDACTED_CREDENTIAL]环境变量已正确加载，但使用该自定义密钥连接模型时始终无法建立连接。使用默认测试密钥[REDACTED_CREDENTIAL]时可正常连接。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1168)
