---
title: 解决FastGPT调用自定义OpenAI格式接口返回为空问题
slug: /zh/troubleshoot/fastgpt-custom-openai-api-empty-response
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/420
source_type: GitHub issue
---

# 解决FastGPT调用自定义OpenAI格式接口返回为空问题
...
> 用户在FastGPT中配置符合OpenAI标准结构的第三方自定义接口，该接口的请求接收结构与官方OpenAI chat completions接口一致，返回结构也严格遵循官方格式。但在FastGPT中调用该接口时，始终返回为空，无法获取到接口返回的结果。用户确认自定义接口本身可正常工作，且其他符合标准的同类接口调用无异常。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/420)
