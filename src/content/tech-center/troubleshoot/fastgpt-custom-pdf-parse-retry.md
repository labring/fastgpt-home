---
title: FastGPT重试Agent调用时自定义PDF解析不生效的排错方法
slug: /zh/troubleshoot/fastgpt-custom-pdf-parse-retry
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3440
source_type: GitHub issue
---

# FastGPT重试Agent调用时自定义PDF解析不生效的排错方法

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.15-fix3；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT V4.8.15-fix3私有部署版本中，用户按照升级文档配置自定义PDF marker的环境变量并调试成功后，在流程调试状态下，点击用户消息右上角的重试按钮发起Agent调用时，系统未调用自定义文档解析功能，仅使用内置的文档解析功能。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3440)
