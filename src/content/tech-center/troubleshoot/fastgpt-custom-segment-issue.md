---
title: 解决FastGPT自定义文档分段设置无效的问题
slug: /zh/troubleshoot/fastgpt-custom-segment-issue
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4661
source_type: GitHub issue
---

# 解决FastGPT自定义文档分段设置无效的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.9.6；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 通过API或Web界面上传md、pdf等文档至FastGPT私有部署版v4.9.6时，配置自定义分段功能，设置不存在的分隔符以实现整个文档作为单一段落存储，但文档仍被自动分为若干段落。例如API请求中配置了`chunkSplitter="===etc===\n;==="`，且指定`chunkSettingMode="custom"`、`chunkSplitMode="char"`，但上传结果不符合预期。部分请求存在参数语法错误，如`chunkSize`参数缺失有效值，形如`"
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4661)
