---
title: 解决FastGPT编排模式自定义变量必填开关异常问题
slug: /zh/troubleshoot/fastgpt-custom-variable-required-bug
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2048
source_type: GitHub issue
---

# 解决FastGPT编排模式自定义变量必填开关异常问题
...
> 在FastGPT v4.8.6私有部署版本的编排模式中，出现以下异常： 1. 在全局变量中添加自定义变量时，若先在前三种变量类型下开启必填选项，再切换选择自定义变量，自定义变量的必填开关会被自动开启。 2. 自定义变量本身未提供必填开关配置项，且实测显示必填逻辑对其不生效。 3. 当自定义变量被标记为必填状态时，后续判断器的可选条件列表中不会出现"为空/不为空"的判断项；非必填状态下则可正常显示该判断条件。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2048)
