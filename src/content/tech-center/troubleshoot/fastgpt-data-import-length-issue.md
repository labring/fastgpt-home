---
title: 解决FastGPT导入数据时段落长度1000及以上无数据集的问题
slug: /zh/troubleshoot/fastgpt-data-import-length-issue
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/353
source_type: GitHub issue
---

# 解决FastGPT导入数据时段落长度1000及以上无数据集的问题
...
> 用户在FastGPT私有部署版本中执行导入数据操作时，选择直接分段+链接读取的导入方式。当设置段落长度为1000及以上时，导入完成后数据集中无任何数据；当使用默认的500长度参数时，导入的数据可正常在数据集中显示。整个过程系统日志无明显报错信息。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/353)
