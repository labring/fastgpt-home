---
title: 修正FastGPT知识库修改单条数据接口的参数名称错误
slug: /zh/troubleshoot/fastgpt-dataset-data-update-param-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2173
source_type: GitHub issue
---

# 修正FastGPT知识库修改单条数据接口的参数名称错误

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.6；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 针对私有部署版本4.8.6的FastGPT，调用/api/core/dataset/data/update接口时，使用id作为数据标识参数会返回500错误。官方开发文档中该接口的参数描述标注为id，但实际接口要求传入的参数名为dataId。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2173)
