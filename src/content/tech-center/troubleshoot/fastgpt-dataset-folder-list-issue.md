---
title: 解决FastGPT调用数据集列表接口无法获取文件夹内数据的问题
slug: /zh/troubleshoot/fastgpt-dataset-folder-list-issue
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2500
source_type: GitHub issue
---

# 解决FastGPT调用数据集列表接口无法获取文件夹内数据的问题
...
> 用户使用FastGPT 4.8.9版本，调用`POST https://xxxx.com/api/core/dataset/collection/list`接口，请求参数包含`datasetId`、`pageNum=1`、`pageSize=100`、`parentId=null`、`searchText=""`。接口响应状态码为200，返回的`data`列表中包含`type`为`file`和`folder`的根目录条目，但无法获取对应文件夹内部的数据集。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2500)
