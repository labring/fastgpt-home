---
title: 解决FastGPT工具调用时输出大模型思考过程的问题
slug: /zh/troubleshoot/fastgpt-disable-tool-call-thought-output
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1811
source_type: GitHub issue
---

# 解决FastGPT工具调用时输出大模型思考过程的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.4-fix、0.5.10、0.12.1；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在私有部署的FastGPT 4.8.4-fix版本中，通过oneapi 0.5.10接入xinference 0.12.1部署的Qwen2-72B-Instruct-GPTQ-Int4模型，使用工具调用功能时，对话界面会输出大模型的思考过程，不符合预期。该问题关联issue#1668，用户提供了两张对话截图与xinference运行日志片段，同时给出了模型的config.json配置信息。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1811)
