---
title: 解决FastGPT私有部署时Docker镜像构建失败的问题
slug: /zh/troubleshoot/fastgpt-docker-build-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5888
source_type: GitHub issue
---

# 解决FastGPT私有部署时Docker镜像构建失败的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.14.0-fix、v4.13.2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户通过Codespaces拉取FastGPT v4.14.0-fix、v4.13.2版本的源码，未做任何修改，直接执行`docker build -f ./projects/app/Dockerfile -t fastgpt . --build-arg name=app`命令构建Docker镜像时，出现构建报错，镜像构建失败。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5888)
