---
title: 解决FastGPT私有部署Docker构建pnpm安装依赖超时问题
slug: /zh/troubleshoot/fastgpt-docker-pnpm-timeout-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3064
source_type: GitHub issue
---

# 解决FastGPT私有部署Docker构建pnpm安装依赖超时问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.12-beta、12.1.3、4.1.0、4.17.7、4.0.9、4.61.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 私有部署FastGPT 4.8.12-beta版本时，构建Docker镜像的第19行`RUN pnpm i`步骤出现依赖安装超时。报错信息包含`ERR_SOCKET_TIMEOUT`，无法下载`openapi-types-12.1.3.tgz`、`js-yaml-4.1.0.tgz`、`@types/lodash-4.17.7.tgz`、`@types/js-yaml-4.0.9.tgz`、`openai-4.61.0.tgz`等npm包，最终因`request to htt
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3064)
