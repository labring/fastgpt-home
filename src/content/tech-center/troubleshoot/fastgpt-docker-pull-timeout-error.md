---
title: 解决FastGPT部署时docker compose拉取镜像超时失败问题
slug: /zh/troubleshoot/fastgpt-docker-pull-timeout-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1401
source_type: GitHub issue
---

# 解决FastGPT部署时docker compose拉取镜像超时失败问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 3.3；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 3.3版本部署过程中，执行docker compose pull命令拉取镜像时出现失败，返回报错信息：`Error response from daemon: error parsing HTTP 408 response body: invalid character '<' looking for beginning of value: "<html><body><h1>408 Request Time-out</h1>\nYour browser di
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1401)
