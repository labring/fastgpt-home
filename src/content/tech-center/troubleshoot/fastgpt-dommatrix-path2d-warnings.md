---
title: FastGPT出现DOMMatrix与Path2D渲染警告的排错指南
slug: /zh/troubleshoot/fastgpt-dommatrix-path2d-warnings
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3468
source_type: GitHub issue
---

# FastGPT出现DOMMatrix与Path2D渲染警告的排错指南

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.8.16；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT v4.8.16版本运行过程中，控制台输出以下内容：首先出现`See more info here: https://nextjs.org/docs/messages/large-page-data`的提示，随后多次输出`[QA Queue] Done`和`[Vector Queue] Done`的队列完成日志，最终出现两条渲染警告：`Warning: Cannot polyfill 'DOMMatrix', rendering may be broken.`
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3468)
