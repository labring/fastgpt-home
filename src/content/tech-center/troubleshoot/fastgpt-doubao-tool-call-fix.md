---
title: 解决FastGPT私有部署版豆包模型工具调用失效的问题
slug: /zh/troubleshoot/fastgpt-doubao-tool-call-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3489
source_type: GitHub issue
---

# 解决FastGPT私有部署版豆包模型工具调用失效的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.15；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用FastGPT私有部署4.8.15版本时，豆包模型本身具备Tool Call能力，但在FastGPT的工具调用功能中无法正常触发工具调用。通过curl调用豆包模型接口查看返回结果，发现当触发tool_calls时，返回的messages.content包含选择工具的原因文本，且存在tool_calls字段，finish_reason为"tool_calls"，但FastGPT未正确识别该响应类型，将其当作普通助手回复处理。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3489)
