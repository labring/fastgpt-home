---
title: 解决FastGPT 4.8.10-fix版本中duckduckgo搜索VQD获取失败问题
slug: /zh/troubleshoot/fastgpt-duckduckgo-vqd-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2683
source_type: GitHub issue
---

# 解决FastGPT 4.8.10-fix版本中duckduckgo搜索VQD获取失败问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.10-fix；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.8.10-fix版本中，使用duckduckgo搜索工具时会触发报错，完整报错文本为`Error: Failed to get the VQD for query "google怎么访问"`。该版本下时间插件、网页抓取插件可正常运行，图片、新闻、视频搜索功能存在异常。同时日志显示`Worker systemPluginRun run time: 11116ms`，`/api/core/chat/chatTest`请求耗时14104ms，请求完成后返回`t
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2683)
