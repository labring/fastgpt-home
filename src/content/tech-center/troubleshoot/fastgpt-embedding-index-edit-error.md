---
title: 解决FastGPT 4.8.5及以上版本嵌入模型索引与编辑异常问题
slug: /zh/troubleshoot/fastgpt-embedding-index-edit-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2342
source_type: GitHub issue
---

# 解决FastGPT 4.8.5及以上版本嵌入模型索引与编辑异常问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.5、4.8.4；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.8.5及以上版本中，使用百度千帆平台的tao-8k嵌入模型时，会出现两类异常：一是上传表格文件后，索引流程一直处于"索引中"状态；二是手动添加或修改知识片时，同时编辑主要内容与辅助数据或索引，会触发"Embedding API is not responding"报错。将版本回退至4.8.4后，上述异常均消失，可正常同时修改知识片内容与辅助数据。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2342)
