---
title: 解决FastGPT使用Embedding-3-large时向量维度超限报错问题
slug: /zh/troubleshoot/fastgpt-embedding3-large-dimension-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2416
source_type: GitHub issue
---

# 解决FastGPT使用Embedding-3-large时向量维度超限报错问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.9；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用私有部署4.8.9版本的FastGPT，绑定单一知识库并使用Embedding-3-large模型，开启混合搜索与模型重排功能后，在模型重排环节前出现报错。系统日志重复输出提示：`The current vector dimension is 3072, and the vector dimension cannot exceed 1536. The first 1536 dimensions are automatically captured`，最终出现`rera
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2416)
