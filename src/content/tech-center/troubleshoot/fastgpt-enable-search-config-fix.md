---
title: 解决FastGPT私有部署中enable_search配置不生效的问题
slug: /zh/troubleshoot/fastgpt-enable-search-config-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1164
source_type: GitHub issue
---

# 解决FastGPT私有部署中enable_search配置不生效的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.7；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT 4.7私有部署版本中，参考issue#1152的答复，尝试在模型配置的defaultConfig字段中配置enable_search参数以启用搜索功能，先后使用了"enable_search": "True"、"enable_search": "true"、"enable_search": True、"enable_search": true四种格式，均未达到预期效果。同时用户无法明确“允许传入自定义参数”对应的具体配置项位置。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1164)
