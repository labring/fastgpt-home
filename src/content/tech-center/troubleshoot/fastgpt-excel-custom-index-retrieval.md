---
title: 解决FastGPT中Excel上传自定义索引后检索不准确的问题
slug: /zh/troubleshoot/fastgpt-excel-custom-index-retrieval
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2840
source_type: GitHub issue
---

# 解决FastGPT中Excel上传自定义索引后检索不准确的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 5.4.3；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户上传格式为`dataId,q,a,index1,index2`的Excel文件至FastGPT知识库，文件包含自定义索引字段。上传完成后，使用自定义索引关键词检索对应知识库段落时无法匹配，例如搜索“5.4.3霍尔果斯市伊车嘎善乡（伊车嘎善村）高标准农田建设项目施工方法”未返回目标条目，需手动重新添加该条目的自定义索引后才能正常检索。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2840)
