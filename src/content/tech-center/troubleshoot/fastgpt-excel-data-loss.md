---
title: 解决FastGPT知识库上传Excel文件后数据不完整的问题
slug: /zh/troubleshoot/fastgpt-excel-data-loss
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4922
source_type: GitHub issue
---

# 解决FastGPT知识库上传Excel文件后数据不完整的问题
...
> 在FastGPT V4.9.10公有云版本中，向知识库添加已上传的Excel文件时，分块预览阶段显示正常（如82个分块），但正式上传后索引仅保留表头，数据未完整上传。部分5000行的Excel文件上传后仅剩余约3000条数据，调整分块条件后剩余数据进一步减少，但用户确认每行数据均无异常。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4922)
