---
title: 解决FastGPT加入额外网络后无法通过该网络IP访问的问题
slug: /zh/troubleshoot/fastgpt-extra-network-access-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4014
source_type: GitHub issue
---

# 解决FastGPT加入额外网络后无法通过该网络IP访问的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.9.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用私有部署版本V4.9.0的FastGPT，将独立运行的Nginx容器置于nginx_net网络中，将FastGPT容器追加加入该网络并指定IP后，无法通过该网络的IP访问FastGPT服务。进入FastGPT容器后发现，服务仅绑定在了FastGPT自身默认网络的IP上。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4014)
