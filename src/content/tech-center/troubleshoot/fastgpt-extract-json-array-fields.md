---
title: FastGPT HTTP调用后提取JSON数组与指定字段的方法
slug: /zh/troubleshoot/fastgpt-extract-json-array-fields
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1159
source_type: GitHub issue
---

# FastGPT HTTP调用后提取JSON数组与指定字段的方法
...
> 用户使用HTTP功能调用必应搜索API进行新闻查询，需要获取API返回结果中的`webPages.value`数组。用户疑惑是否只能通过`webPages.value[0]`、`webPages.value[1]`这样的逐个索引方式调用，希望能一次性传出该数组。同时用户还希望提取数组中每个元素的`snippet`和`url`字段，不清楚该如何配置字段提取规则。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1159)
