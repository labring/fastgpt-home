---
title: 解决FastGPT对话中多个音视频标签仅单个可用的问题
slug: /zh/troubleshoot/fastgpt-fix-multiple-audio-video
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3734
source_type: GitHub issue
---

# 解决FastGPT对话中多个音视频标签仅单个可用的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.20-fix2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.8.20-fix2版本中，对话输出包含多个音视频标签时，仅首个标签可正常渲染并播放，其余标签显示为灰色不可点击状态。即使首个标签播放完成，其余标签仍无法操作。通过浏览器控制台检查可见，除首个标签外，其余标签的src属性为空，未被正确填充资源。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3734)
