---
title: 解决FastGPT调用Ollama嵌入模型时的异常端口配置问题
slug: /zh/troubleshoot/fastgpt-fix-ollama-port-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3241
source_type: GitHub issue
---

# 解决FastGPT调用Ollama嵌入模型时的异常端口配置问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.14、127.0.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用FastGPT 4.8.14版本，在调用Ollama嵌入模型时，embedding接口可用，但FastGPT调用了127.0.0.1:40476/health端口，该端口未在Ollama服务端出现。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3241)
