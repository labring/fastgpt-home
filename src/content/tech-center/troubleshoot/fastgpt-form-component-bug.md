---
title: 解决FastGPT V4.9.0-fix2版表单组件异常问题
slug: /zh/troubleshoot/fastgpt-form-component-bug
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4104
source_type: GitHub issue
---

# 解决FastGPT V4.9.0-fix2版表单组件异常问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.9.0-fix2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 当前使用ghcr.io/labring/fastgpt:V4.9.0-fix2私有部署版本时，存在两个表单组件异常问题：一是数字输入框组件配置的默认值无法正常显示；二是单选框组件无法单独设置下拉选项的value值，当前仅能将选项的text内容作为默认的value值。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4104)
