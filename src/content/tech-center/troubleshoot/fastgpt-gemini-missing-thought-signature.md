---
title: 解决FastGPT中Gemini模型调用时未代入思维链签名的问题
slug: /zh/troubleshoot/fastgpt-gemini-missing-thought-signature
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/6135
source_type: GitHub issue
---

# 解决FastGPT中Gemini模型调用时未代入思维链签名的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.14.3；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT私有部署4.14.3版本中，使用Gemini-3-Flash或Gemini-3-Pro模型时，配置工具调用连接MCP-Sever与知识库检索节点后，本地打印的LLM请求日志未包含thought_signature参数。使用其他模型时可正常生成回复，且日志包含该参数。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/6135)
