---
title: 解决FastGPT私有部署版Gemini工具调用函数名校验报错
slug: /zh/troubleshoot/fastgpt-gemini-tool-name-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5043
source_type: GitHub issue
---

# 解决FastGPT私有部署版Gemini工具调用函数名校验报错

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.9.10、2.5-pro；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 使用FastGPT私有部署V4.9.10版本，选择Gemini系列模型（如gemini-2.5-pro-preview-06-05），启用工具调用插件并添加工具后，发起聊天时出现报错。报错提示为Gemini对工具函数名进行校验，拒绝不符合格式的名称。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5043)
