...
import type { AdminCustomerItem } from '@/customers/components/admin/customer-list/types';
import {
  getAdminCustomerCategoryColor,
  getAdminCustomerCategoryName
} from '@/customers/components/admin/customer-list/utils';

interface TrashCustomerCardProps {
...
function formatDeletedSource(source: AdminCustomerItem['deletedSource']) {
  if (source === 'agent') {
    return 'Agent 删除';
  }
...
function formatDateTime(value: AdminCustomerItem['deletedAt']) {
  if (!value) {
    return '未知时间';
  }
...
export default function TrashCustomerCard({
  item,
  onRestore,
  onDeletePermanently,
  isRestoring,
  isDeleting
}: TrashCustomerCardProps) {
