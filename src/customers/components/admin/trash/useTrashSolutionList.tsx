...
import {
  clearTrash,
  deleteSolutionFromTrash,
  restoreSolutionFromTrash
} from '@/app/customers/admin/actions/solutions';
import { filterAdminSolutions } from '@/customers/lib/solution-search';
import type { AdminSolutionListData } from '@/customers/components/admin/solution-list/types';

interface UseTrashSolutionListProps {
  initialData: AdminSolutionListData;
}

export function useTrashSolutionList({ initialData }: UseTrashSolutionListProps) {
  ...
  const filteredItems = useMemo(
    () => filterAdminSolutions(data.items, 'all', search, 'time'),
    [data.items, search]
  );
  ...
  const handleDeletePermanently = useCallback(async () => {
    ...
    const result = await deleteSolutionFromTrash(pendingDeleteId);
    setDeletingId('');
