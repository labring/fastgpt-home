...
export function buildAiSummaryCacheKey(customerId: string, contentVersion: string | undefined, key: string) {
  const version = contentVersion?.trim() || 'current';
  return `ai_cache_${customerId}_${version}_${key}`;
}

export function readAiSummaryCache(customerId: string, contentVersion?: string): AiSummaryCacheSnapshot {
  ...
  const getCacheKey = (key: string) => buildAiSummaryCacheKey(customerId, contentVersion, key);
...
export default function AiSummaryCard({ customerId, contentVersion }: AiSummaryCardProps) {
  ...
  // Stop generation when customerId changes
  useEffect(() => {
    return () => {
