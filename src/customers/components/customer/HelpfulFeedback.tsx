...
const VOTE_STORAGE_EVENT = 'customer-vote-storage';

function getVoteStorageKey(customerId: string) {
  return `customer_vote_${customerId}`;
}

function readSavedVote(customerId: string): SavedVote {
  if (typeof window === 'undefined') {
    return null;
  }

  const savedVote = localStorage.getItem(getVoteStorageKey(customerId));
  return savedVote === 'helpful' || savedVote === 'unhelpful' ? savedVote : null;
}

function subscribeSavedVote(customerId: string, callback: () => void) {
  if (typeof window === 'undefined') {
    return () => {};
  }

  const handleStorage = (event: StorageEvent) => {
    if (!event.key || event.key === getVoteStorageKey(customerId)) {
      callback();
    }
  };
...
function writeSavedVote(customerId: string, vote: Exclude<SavedVote, null>) {
  localStorage.setItem(getVoteStorageKey(customerId), vote);
  window.dispatchEvent(new Event(VOTE_STORAGE_EVENT));
}

export default function HelpfulFeedback({ customerId }: { customerId: string }) {
  const voted = useSyncExternalStore(
    (callback) => subscribeSavedVote(customerId, callback),
    () => readSavedVote(customerId),
    () => null
  );
  ...
  const handleVote = async (type: 'helpful' | 'unhelpful') => {
    ...
    try {
      const res = await fetch(withBasePath(`/api/customers/${customerId}/vote`), {
        method: 'POST',
        headers: {
      ...
      const savedType = data?.votedType === 'helpful' || data?.votedType === 'unhelpful'
        ? data.votedType
        : type;
      writeSavedVote(customerId, savedType);
