...
import CustomerCard, { type Customer } from '@/customers/components/CustomerCard';
...
interface CustomersSectionProps {
  ...
  customers: Customer[];
  isLoading: boolean;
  isShowingStaleCustomers: boolean;
          ...
          <div className="relative">
            <div
              className={`absolute left-0 right-0 -top-3 z-10 h-0.5 overflow-hidden rounded-full bg-brand-100 transition-opacity duration-200 dark:bg-[#203652] ${
                isShowingStaleCustomers ? 'opacity-100' : 'opacity-0'
              }`}
              aria-hidden="true"
            >
            ...
            <div
              className={`grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3 transition-opacity duration-200 ${
                isShowingStaleCustomers ? 'opacity-70' : 'opacity-100'
              }`}
              aria-busy={isShowingStaleCustomers}
            >
          ...
          {hasMoreCustomers && (
            <div className="mt-8 flex justify-center">
              <button
                type="button"
                onClick={onLoadMore}
                disabled={isLoadingMore || isCustomersLoading}
