...
import { normalizeCustomerFolderName } from '@/customers/lib/customer-storage';
...
export function resolveStorageFolder(input: unknown) {
  return typeof input === 'string' ? normalizeCustomerFolderName(input) : '';
}

export function normalizeAdminCustomerListItems<T extends AdminCustomerListItem>(items: T[]): T[] {
