...
export function warmSolutionImages(solutions: Solution[], warmedImageUrls: Set<string>) {
  if (typeof window === 'undefined') {
    return;
  }

  solutions.slice(0, 4).forEach((solution) => {
    const src = solution.thumbnailUrl || solution.imageUrl;
