...
import { allocateUntitledSolutionFolder } from '@/customers/lib/solution-storage.server';
...
function revalidateSolutionViews(solutionId?: string, categorySlug?: string, previousCategorySlug?: string) {
  revalidateAdminRouteTree();
  revalidateSolutionRefs(solutionId ? { id: solutionId, categorySlug, previousCategorySlug } : []);
}
...
function buildSolutionPersistencePayload({
  data,
  categoryName,
  storageFolder,
  imageUrl,
  ...
  return {
    ...
    slug: data.slug !== undefined ? normalizeSolutionSlug(data.slug) || null : undefined,
    ...
    relatedSolutionIds:
      data.relatedSolutionIds !== undefined ? data.relatedSolutionIds : undefined,
...
async function migrateSolutionMediaFolder({
  sourceFolder,
  targetFolder,
  content,
  imageUrl,
...
export async function createSkeletonSolutionForAgent() {
  ...
  try {
    ...
    const newSol = await Solution.create(payload);
    revalidateSolutionViews(id, category.slug);

    return { success: true, id: newSol._id.toString(), categorySlug: category.slug };
  } catch (error: unknown) {
    console.error('Failed to create skeleton solution:', error);
    return { success: false, error: getErrorMessage(error, '创建失败') };
  }
}

export async function saveSolutionForAgent(data: SaveSolutionInput) {
  await dbConnect();
  try {
    if (!data.id) {
    ...
    const id = data.id;
    const [category, existingSolution] = await Promise.all([
      Category.findById(data.categoryId),
      Solution.findById(id).populate('categoryId', 'slug')
    ]);
    ...
    const previousCategory =
      typeof existingSolution.categoryId === 'object' && existingSolution.categoryId !== null
        ? existingSolution.categoryId as { slug?: string | null }
        : null;
    const previousCategorySlug = previousCategory?.slug || undefined;

    const slug = data.slug !== undefined ? normalizeSolutionSlug(data.slug) : undefined;
    if (slug !== undefined && slug && !isValidSolutionSlug(slug)) {
      return { success: false, error: 'Slug 只能包含小写字母、数字和连字符' };
    }
    if (slug && !(await isSolutionSlugAvailable(slug, { excludeId: id }))) {
      return { success: false, error: 'Slug 已存在，请更换为唯一值' };
    }

    const targetStorageFolder = existingSolution.storageFolder || id;
    const sourceStorageFolder = resolveStorageFolder(data.storageFolder) || targetStorageFolder;

    const migrationResult = await migrateSolutionMediaFolder({
    ...
    const payload = buildSolutionPersistencePayload({
      data,
      categoryName: category.name,
    ...
    const urlsToDelete = diffRemovedMediaUrls({
      currentUrls,
      previousUrls: existingSolution.mediaUrls || [],
    ...
    const updateResult = await Solution.findOneAndUpdate(
      { _id: id },
      payload,
    ...
    revalidateSolutionViews(id, category.slug, previousCategorySlug);
