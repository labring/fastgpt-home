...
export interface SolutionCoverSearchInput {
  title?: string;
  description?: string;
  content?: string;
...
export interface SolutionCoverSearchResult {
  imageUrl: string;
  thumbnailUrl: string;
  query: string;
  reason: string;
...
export function buildSolutionCoverFallbackQuery({
  title,
  description,
  content
}: Required<Pick<SolutionCoverSearchInput, 'title' | 'description' | 'content'>>) {
...
async function generateCoverSearchQuery(payload: {
  ...
  const fallbackQuery = buildSolutionCoverFallbackQuery(payload);
  ...
  return generateAISearchQuery({
    ...
    systemPrompt:
      'You turn Chinese solution descriptions into concrete English Pexels photo search phrases for landscape website hero covers.',
...
export async function generateSolutionCover({
  title = '',
  description = '',
  content = '',
  storageFolder
}: SolutionCoverSearchInput): Promise<SolutionCoverSearchResult> {
  ...
  const assetResult = await searchPexelsAssetWithFallback({
    ...
    fallbackQuery: buildSolutionCoverFallbackQuery({
      title: normalizedTitle,
      description: normalizedDescription,
      content: normalizedContent
    })
