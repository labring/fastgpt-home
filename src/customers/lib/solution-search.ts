...
import type { PublicSolutionSortKey } from '@/customers/lib/solution-pagination';
...
export type SolutionSortKey = PublicSolutionSortKey;
export type AdminSolutionSortKey = SolutionSortKey | 'updated' | 'draft';
export const DEFAULT_ADMIN_SOLUTION_SORT_KEY: AdminSolutionSortKey = 'updated';

export type SortOptionIcon = 'Heart' | 'Eye' | 'Clock' | 'PencilSimple' | 'FileDashed';

export interface SortOption<TSortKey extends string = SolutionSortKey> {
  id: TSortKey;
  label: string;
  icon: SortOptionIcon;
  color: string;
...
export const ADMIN_SORT_OPTIONS: SortOption<AdminSolutionSortKey>[] = [
...
export function isAdminSolutionSortKey(value: string | null | undefined): value is AdminSolutionSortKey {
  return ADMIN_SORT_OPTIONS.some((option) => option.id === value);
}

export function normalizeAdminSolutionSortBy(value: string | null | undefined): AdminSolutionSortKey {
  return isAdminSolutionSortKey(value) ? value : DEFAULT_ADMIN_SOLUTION_SORT_KEY;
}
...
export function filterPublicSolutions(
  ...
  return solutions
    .filter((solution) => {
      ...
      return (
        matchesCategory &&
        matchesSolutionSearch(solution.title, solution.description, searchQuery)
      );
    })
    .sort((left, right) => {
      if (sortBy === 'likes') {
...
export function filterAdminSolutions<
  T extends {
    title?: string;
    description?: string;
...
>(items: T[], currentCategory: string, searchQuery: string, sortBy = DEFAULT_ADMIN_SOLUTION_SORT_KEY) {
...
export async function fetchFirstMatchedSolution<T = unknown>(search: string) {
