...
import { extractS3KeyFromPublicUrl } from '@/customers/lib/solution-storage';
import { buildThumbnailKey } from '@/customers/lib/image-thumbnail';
import { isValidObjectId } from '@/customers/lib/object-id';

export type SolutionDeleteSource = 'admin' | 'agent';
...
function buildFindIncludingDeletedQuery(id: string) {
  return { _id: id, deletedAt: { $exists: true } };
}

async function deleteSolutionAssets(solution: TrashableSolution) {
  const { client, bucket, publicUrlBase } = await getS3Client();

  const urlsToDelete = [
    ...(solution.imageUrl && solution.imageUrl !== '/fastgpt.svg' ? [solution.imageUrl] : []),
    ...(solution.thumbnailUrl && solution.thumbnailUrl !== '/fastgpt.svg' ? [solution.thumbnailUrl] : []),
    ...cleanMediaUrls(solution.mediaUrls || []),
    ...extractMediaUrlsFromMarkdown(solution.content || '')
  ];
  ...
  try {
    ...
    console.error('Failed to delete solution assets from S3:', error);
  }
}

async function deleteTrashedSolutionDocument(id: string) {
  const result = await Solution.deleteOne({
    _id: id,
    deletedAt: { $ne: null }
  });
...
function revalidateTrashMutation(refs: SolutionRevalidationRef[]) {
...
export async function getTrashedSolutions(search = '') {
  ...
  return {
    items: normalizeAdminSolutionListItems(serializedSolutions),
    total: solutions.length
  };
}

export async function moveSolutionToTrash(id: string, deletedSource: SolutionDeleteSource) {
  if (!isValidObjectId(id)) {
    return { success: false as const, error: '案例不存在或已在回收站中' };
  }

  await dbConnect();
  const refs = await loadSolutionRevalidationRefs([id]);
...
export async function moveSolutionsToTrash(ids: string[], deletedSource: SolutionDeleteSource) {
  await dbConnect();
  ...
  const matchedIds = existingSolutions.map((item) => String(item._id));
...
export async function permanentlyDeleteSolutionFromTrash(id: string) {
  if (!isValidObjectId(id)) {
    return { success: false as const, error: '案例不存在或未在回收站中' };
  }
  ...
  const solution = await Solution.findOne({
    _id: id,
    deletedAt: { $ne: null }
  }).lean<TrashableSolution | null>();

  if (!solution) {
    return { success: false as const, error: '案例不存在或未在回收站中' };
  }

  await deleteSolutionAssets(solution);
  const deleted = await deleteTrashedSolutionDocument(id);
  if (!deleted) {
...
export async function emptyTrash() {
  ...
  const trashedSolutions = await Solution.find({ deletedAt: { $ne: null } })
    .select('_id imageUrl thumbnailUrl mediaUrls content')
    .lean<TrashableSolution[]>();
  const refsById = new Map(
    (await loadSolutionRevalidationRefs(trashedSolutions.map((solution) => String(solution._id)))).map((ref) => [
      ref.id,
      ref
    ])
  );
  ...
  const deletedRefs: SolutionRevalidationRef[] = [];
  let deletedCount = 0;

  for (const solution of trashedSolutions) {
    ...
    try {
      ...
      failedIds.push(id);
      console.error(`Failed to permanently delete trashed solution ${id}:`, error);
    }
  }
...
export async function getSolutionByIdIncludingDeleted(id: string) {
  if (!isValidObjectId(id)) {
    return null;
  }

  await dbConnect();
  return Solution.findOne(buildFindIncludingDeletedQuery(id));
}
