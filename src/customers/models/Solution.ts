...
SolutionSchema.index({ categoryName: 1, isPublished: 1, deletedAt: 1, createdAt: -1 });

// 案例页列表与相关推荐
SolutionSchema.index({ contentType: 1, isPublished: 1, deletedAt: 1, createdAt: -1 });
SolutionSchema.index({ categoryName: 1, isPublished: 1, deletedAt: 1, publishedAt: -1 });

// 全量已发布内容查询（sitemap / llms / 首页 AI 目录 / 相关推荐）：发布 + 未删除 + 时间排序
SolutionSchema.index({ isPublished: 1, deletedAt: 1, createdAt: -1 });

// 分类过滤列表查询：分类 + 发布 + 未删除 + 时间排序（sortBy=time 时排序走索引）
SolutionSchema.index({ categoryId: 1, isPublished: 1, deletedAt: 1, createdAt: -1 });

// 虚拟属性：格式化 usageCount (例如：12000 -> 1.2w+)
SolutionSchema.virtual('formattedUsageCount').get(function(this: ISolution) {
...
// 查询中间件：默认过滤掉已软删除的文档
SolutionSchema.pre(/^find/, function(this: mongoose.Query<unknown, ISolution>) {
